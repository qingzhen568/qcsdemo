...
class Cart extends Component{
	...
	render(){
		return (
			<div className="cart">
				<h1 className="cart_title">购物车</h1>
				<div className="cart_content">
				{
					this.state.shopType?<ul>
					{
						this.state.shopArr.map((item,index)=><li>
							<img src={item.img_src} alt={item.name}/>
							{item.name}<br/>
							{item.app_price} {item.market_price}  {item.num}
						</li>)
					}
					</ul>:<div className ="cart-tishi">
						<div>	
						...
						<div>
							<Button type="primary" className="cart_btn" onClick={this.goHome}>去首页逛逛</Button>
						</div>
					</div>
				}
				</div>
